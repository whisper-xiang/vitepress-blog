...
基本类型也称为简单类型，由于其占据空间固定，为了便于提升变量查询速度，将其存储在栈（Stack）中。 基础类型的值是不可变的，一旦创建，其值本身就不能改变。相反，改变原始数据类型的值通常会创建一个新的值。
...
但要注意的是， `String` 的意义并非“字符串”，而是字符串的 `UTF16` 编码，我们字符串的操作 `charAt`、`charCodeAt`、`length` 等方法针对的都是 `UTF16` 编码。所以，字符串的最大长度，实际上是受字符串的编码长度影响的。
...
- [字符、代码单元和码点](https://juejin.cn/post/6844904024215404558)
  :::
...
- 双精度浮点数在内存中占 8 个字节、有效数字 16 位、表示范围：-1.79E+308 ~ +1.79E+308
  ...
  **浮点型简单来说就是表示带有小数的数据，而恰恰小数点可以在相应的二进制的不同位置浮动，可能是这样就被定义成浮点型了**。
  ...
  JavaScript 中所有数字包括整数和小数都只有一种类型 即 Number 类型。它的实现遵循  **IEEE 754 标准**， 以 64 位双精度浮点数存储所有 Number 类型值 即计算机最多存储 64 位二进制数。(64 位 = 符号位+指数位+小数部分)
...
你也许有过疑问，明明基础类型的变量，为什么会有方法可以被调用？
比如：
