...
`String` 用于表示文本数据。`String` 有最大长度是 **2^53 - 1** 个字符，这是由 `JavaScript` 使用的数字数据类型的安全整数范围 (Number.MAX_SAFE_INTEGER) 决定的。 要注意的是， `String` 的意义并非“字符串”，而是字符串的 `UTF16` 编码，我们字符串的操作 `charAt`、`charCodeAt`、`length` 等方法针对的都是 `UTF16` 编码。所以，字符串的最大长度，实际上是受字符串的编码长度影响的。
...
- 字符编码方式: 将字符映射为计算机可以处理和存储的数字（通常是字节或字节序列）的规则。
  - UTF-16: 是一种定长与可变长度相结合的字符编码方式，通常使用 2 个字节（16 位）来表示字符，但对于更高的字符，它使用一对 16 位的代码单元。在此编码规则下，大部分常见的汉字都是用 2 个字节（16 位）表示的。
  - Unicode: 是一种字符编码标准，它将每一个字符都分配一个唯一的码位，使得每个字符都能用唯一的数字来表示。
  - UTF-8: 是一种变长编码方式，它使用 1-6 个字节来表示字符，可以表示更多的字符。
    :::
...
`JavaScript` 的 `Number` 类型为[双精度 IEEE 754 64 位浮点](https://en.wikipedia.org/wiki/Floating-point_arithmetic)类型。 有 **18437736874454810627** (即 2^64-2^53+3) 个值。
...
- NaN (not a number：非数字值，用于表示某些无法计算结果的情况，比如 0/0 或者 Infinity/Infinity；)
- Infinity，无穷大；
- -Infinity，负无穷大。

另外，值得注意的是，JavaScript 中有 `+0` 和 `-0`，在加法类运算中它们没有区别，但是除法的场合则需要特别留意区分。
...
浮点数的精度丢失是指，由于浮点数的存储机制，导致了浮点数的小数部分的精度丢失。比如：
...
- 字面量：`{}`、`[]`、`new Date()`、`new RegExp()`、`Math`、`JSON`
...
- 构造函数：`new Object()`、`new Array()`、`new Date()`、`new RegExp()`、`new Error()`、`new Map()`、`new Set()`、`new WeakMap()`、`new WeakSet()`
...
你也许有过疑问，明明是基础类型的变量，为什么会有方法可以被调用？
比如：
